...
shr = [10, 11, 2, 5, 3, 7, 8, 3, 1, 1, 10, 5]
max_berry = shr[-1] + shr[0] + shr[1]
for i in range(1, len(shr)):
    if i != len(shr) - 1:
        if shr[i - 1] + shr[i] + shr[i + 1] > max_berry:
            max_berry = shr[i - 1] + shr[i] + shr[i + 1]
    else:
        if shr[i - 1] + shr[i] + shr[0] > max_berry:
            max_berry = shr[i - 1] + shr[i] + shr[0]
print(max_berry)